...
class Wargame_env(object):
    ...
    def check_win(self):
        ...
        if self.rounds > 50:
            # 双方夺控分计算
            self.red_all_score = self.red_point_flag * 10 + 30 - blue_tank_all_num
            self.blue_all_score = self.blue_point_flag * 10 + 30 - red_tank_all_num
            # 蓝方总分高
            if self.red_all_score <= self.blue_all_score:
                ...
                self.blue_win_times += 1
                self.total_episodes += 1
                self.blue_pointattack_score += self.blue_all_score
                self.red_pointattack_score += self.red_all_score
                return True
