...
class RTank(Piece):
    ...
    def check_close_to_goal(self):
        ...
        d1 = self.wargame_env.game_map.get_distance_between_hex(old_position[0],old_position[1],self.goal[0],self.goal[1])
        d2 = self.wargame_env.game_map.get_distance_between_hex(self.co_x,self.co_y,self.goal[0],self.goal[1])
        if d1 > d2:
            return 1
        else:
            return 0
    ...
    def get_goal_reward(self):
        ...
        enermy_1_to_goal = self.wargame_env.game_map.get_distance_between_hex(enermy_1.co_x, enermy_1.co_y, self.goal[0], self.goal[1])
        enermy_2_to_goal = self.wargame_env.game_map.get_distance_between_hex(enermy_2.co_x, enermy_2.co_y, self.goal[0], self.goal[1])
    ...
    def change_to_hide_state(self):
        if not self.done:
            ...
            print('invalid red hide operation')
    def change_to_move_state(self):
        self.state = '机动'
    ...
    def direct_fire(self,enermy):
        if not self.done:
            ...
            if observation and distance <= self.fire_range and self.movement == 6:
                ...
                # print('Red_Direct_Fire,kill:',judge_result_mod,'blue save:',enermy.num)
                return judge_result_mod
            else:
                return 0
        else:
            # print('invalid red fire operation')
            return 2
    ...
    def indirect_fire(self,co_x,co_y):
        if not self.done:
            ...
            self.hex_history.append((co_x,co_y))
            self.done = not self.done
            # print('red indirect fire at %s,%s'%(co_x,co_y))
        else:
            print('invalid red indirect fire operation')
    def indirect_fire_judge(self,co_x,co_y):
        ...
        if enermy_1:
            ...
            print('Red_Indirect_Fire,Kill:',result_num,'Blue save:',enermy_1.num)
        if enermy_2:
            result_2 = 0
            result_num = 0
            a1 = random.randint(1, 6)
...
class BTank(Piece):
    def __init__(self, wargame, co_x, co_y, *group):
        ...
        self.movement = 6
        self.actions = ('W', 'E', 'NE', 'NW', 'SE', 'SW', 'HD', 'DF', 'IDF', 'STA')
        self.armor = random.choice(['复合装甲','重型装甲','中型装甲','轻型装甲','无装甲'])
        self.scope_people = 10
        self.scope_vehicle = 25
        self.fire_in_movement_ablity = True
        self.carry_missle_number = 0
        self.fire_range = random.choice([16, 17, 18, 19, 20])
        self.fire_type = random.choice([2, 3, 4, 5])
        self.num = 3
        ...
        self.common_move_cost = 3
        # 2特殊地形所需体力值
        self.special_move_cost = 6
        # 4想定边界长度
        self.map_length = 50
        # 7最大速度
        self.max_speed = random.choice([125, 150, 175, 200])
        # 8机动能力
        self.move_ability = 6
        # 9行进间射击能力
        self.shoot_in_move = 1
    ...
    # 15连线的最高程
    def get_max_height(self, goal):
        n = self.wargame_env.game_map.get_distance_between_hex(self.co_x, self.co_y, goal.co_x, goal.co_y)
        pass_hex = self.wargame_env.game_map.get_hexes_between_hex(self.co_x, self.co_y, goal.co_x, goal.co_y, n)
        height_list = []
        for hex in pass_hex:
            height = self.wargame_env.game_map.get_hex_info(hex[0], hex[1])['ground_id']
            height_list.append(height)
        return max(height_list)
    # 16,17所处地形的实际地形高度
    def get_position_height(self):
    ...
    def check_level_1_street(self):
        ...
        streets = []
        for hex in neighbours:
            cond = self.wargame_env.game_map.get_hex_info(hex[0], hex[1])['grid_id']
            streets.append(cond)
        for num in streets:
            if num in [x for x in range(26, 49)]:
                return 1
    ...
    def check_city_around(self):
        ...
        cities = []
        for hex in neighbours:
            cond = self.wargame_env.game_map.get_hex_info(hex[0], hex[1])['cond']
            cities.append(cond)
        if 3 in cities:
            return 1
        else:
    ...
    def move_one_step(self, co_x, co_y):
        if not self.done:
            ...
            if 10 <= co_x <= 25 and 5 <= co_y <= 45:
                if (co_x,co_y) in self.wargame_env.game_map.get_neighbour(self.co_x,self.co_y):
                    # print('blue tank Move_to: %s, %s'%(co_x,co_y))
            ...
            print('invalid blue move operation')
    ...
    def change_to_hide_state(self):
        if not self.done:
            ...
            print('invalid blue hide operation')
    ...
    def direct_fire(self,enermy):
        if not self.done:
            ...
            if observation and distance <= self.fire_range and self.movement == 6:
                ...
                # print('Blue_Direct_Fire,kill:', judge_result_mod, 'red save:', enermy.num)
                return judge_result
            else:
                return 0
        else:
            print('invalid blue fire operation')
    ...
    def indirect_fire(self, co_x, co_y):
        if not self.done:
            ...
            self.hex_history.append((co_x,co_y))
            self.done = not self.done
            # print('Blue indirect fire at %s,%s'%(co_x,co_y))
        else:
            print('invalid blue indirect fire operation')
    ...
    def indirect_fire_judge(self, co_x, co_y):
        ...
        if enermy_1:
            ...
            if enermy_1.num <= 0:
                enermy_1.num = 0
                # print('Blue_Indirect_Fire,Kill:', result_num, 'Red save:', enermy_1.num)
            if enermy_2:
                result_2 = 0
                result_num = 0
                a1 = random.randint(1, 6)
                ...
                if enermy_2.num <= 0:
                    enermy_2.num = 0
                    # print('Blue_Indirect_Fire,Kill:', result_num, 'Red save:', enermy_2.num)
        ...
        if self.check_win():
            ...
            for neighbour in neighbours:
                if neighbour not in self.move_history:
                    ...
                    d[neighbour] = self.wargame_env.game_map.get_distance_between_hex(self.goal[0], self.goal[1], neighbour[0], neighbour[1])
